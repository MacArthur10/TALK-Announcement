...
// @ts-ignore - types may be missing for storage engine
import { CloudinaryStorage } from "multer-storage-cloudinary";
// @ts-ignore - fichier js personnalisé pour le diagnostic
import { createDiagnosticStorage } from "../utils/storage-cloudinary.js";
...
try {
  ...
  if (!cloudName || !apiKey || !apiSecret) {
    ...
    if (!cloudName) console.error("- CLOUDINARY_CLOUD_NAME manquant");
    if (!apiKey) console.error("- CLOUDINARY_API_KEY manquant");
    if (!apiSecret) console.error("- CLOUDINARY_API_SECRET manquant");
...
try {
  ...
  storage = createDiagnosticStorage({
...
router.post("/test-upload", requireAuth, (req, res) => {
...
router.get("/test-cloudinary", async (_req, res) => {
  ...
  try {
    ...
    console.log("- API Key (premières lettres):", 
      process.env.CLOUDINARY_API_KEY?.substring(0, 4) + "..." + 
      process.env.CLOUDINARY_API_KEY?.substring(process.env.CLOUDINARY_API_KEY.length - 4));
    ...
    res.json({ 
      ...
      config: { 
        ...
        api_key_masked: process.env.CLOUDINARY_API_KEY?.substring(0, 4) + "..." + 
                        process.env.CLOUDINARY_API_KEY?.substring(process.env.CLOUDINARY_API_KEY.length - 4),
...
router.post("/test-direct-cloudinary", requireAuth, async (req, res) => {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const file = await uploadHandler as any;
...
router.post("/test-direct-cloudinary", requireAuth, requireRole(["admin", "superadmin"]), async (req, res) => {
